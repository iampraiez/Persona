...
export class AnalyticsService {
  static async getAnalytics(userId: string, range: string, currentDate: Date) {
    ...
    const skippedReasons = specialEvents.reduce(
    ...
    const specialEventsData = Object.entries(skippedReasons).map(
      ([name, value]) => ({ name, value }),
    );

    const activityData = this.prepareActivityData(range, events, currentDate);
    const goalProgressData = this.prepareGoalProgressData(
      goals,
      startDate,
      endDate,
    );
    ...
    const averageGoalProgress =
      goalProgressData.length > 0
        ? Math.round(
            goalProgressData.reduce((acc, curr) => acc + curr.progress, 0) /
  ...
  private static prepareActivityData(
    ...
    if (range === "day") {
      return Array.from({ length: 24 }, (_, h) => {
        const hourEvents = events.filter(
          (e) => new Date(e.startTime).getHours() === h,
        );
        return {
          name: `${h}:00`,
    ...
    if (range === "week") {
      ...
      return days.map((day, index) => {
        const dayEvents = events.filter(
          (e) => new Date(e.startTime).getDay() === index,
        );
        return {
          name: day,
    ...
    if (range === "year") {
      ...
      return months.map((month, index) => {
        const monthEvents = events.filter(
          (e) => new Date(e.startTime).getMonth() === index,
        );
        return {
          name: month,
    ...
    const years = [
      ...new Set(events.map((e) => new Date(e.startTime).getFullYear())),
    ].sort();
    return years.map((year) => {
      const yearEvents = events.filter(
        (e) => new Date(e.startTime).getFullYear() === year,
      );
      return {
        name: `${year}`,
  ...
  private static prepareGoalProgressData(
    ...
    return goals.map((goal) => {
      ...
      return {
        ...
        progress:
          goal.steps.length > 0
            ? Math.round((completedStepsInRange / goal.steps.length) * 100)
            : 0,
      };
    });
  }
