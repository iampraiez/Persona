...
export class EventController {
  ...
  static async getByDate(req: Request, res: Response) {
    try {
      ...
      logger.error("Get Events by Date Error");
      res.status(500).json({
        data: null,
        error: errorWrapper(error, "Failed to get events"),
      });
  ...
  static async skip(req: Request, res: Response) {
    try {
      ...
      const { id } = req.params;
      const updatedEvent = await EventService.skipEvent(
        userId,
        id as string,
        req.body,
      );
  ...
  static async deleteRange(req: Request, res: Response) {
    try {
      ...
      if (!start || !end)
        return res
          .status(400)
          .json({ error: "Start and end dates are required", data: null });

      await EventService.deleteEventsRange(
        userId,
        start as string,
        end as string,
      );
