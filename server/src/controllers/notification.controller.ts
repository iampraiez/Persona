...
export class NotificationController {
  ...
  static async deleteOne(req: Request, res: Response) {
    try {
      ...
      logger.error(
        `Delete Notification Error: ${error instanceof Error ? error.message : String(error)}`,
      );
      res.status(500).json({
        data: null,
  ...
  static async deleteAll(req: Request, res: Response) {
    try {
      ...
      res
        .status(200)
        .json({ data: "All notifications deleted successfully", error: null });
    } catch (error: unknown) {
  ...
  static async markAsRead(req: Request, res: Response) {
    try {
      ...
      res
        .status(200)
        .json({ data: "Notification marked as read", error: null });
    } catch (error: unknown) {
  ...
  static async markAllRead(req: Request, res: Response) {
    try {
      ...
      res
        .status(200)
        .json({ data: "All notifications marked as read", error: null });
    } catch (error: unknown) {
      logger.error(`Mark All Read Error: ${error}`);
      res.status(500).json({
        data: null,
        error: errorWrapper(error, "Failed to mark all notifications as read"),
      });
    }
  }
