  ...
  cron.schedule("* * * * *", async () => {
    try {
      ...
      for (const event of upcomingEvents) {
        const upcomingNotifyTime = new Date(
          event.startTime.getTime() - event.notifyBefore * 60 * 1000,
        );
        if (
          now >= upcomingNotifyTime &&
          now < new Date(upcomingNotifyTime.getTime() + 60 * 1000)
        ) {
        ...
        if (
          now >= startTime &&
          now < new Date(startTime.getTime() + 60 * 1000)
        ) {
          if (event.user.notificationsEnabled) {
